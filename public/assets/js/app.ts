...
function renderMembers(): void {
    ...
    container.innerHTML = mems.length
        ? mems
              .map(
                  (m) => `
        <div class="card">
...
function renderSchemes(): void {
    ...
    container.innerHTML = schs.length
        ? schs
              .map(
                  (s) => `
        <div class="card">
...
window.deleteInfo = function (id: number) {
    if (prompt("Admin password:") !== ADMIN_PASSWORD) return alert("Incorrect");
    save(KEY_INFO, load<InfoItem>(KEY_INFO).filter((i) => i.id !== id));
    renderInfoList();
};

window.deleteMember = function (id: number) {
    if (prompt("Admin password:") !== ADMIN_PASSWORD) return alert("Incorrect");
    save(KEY_MEMBERS, load<Member>(KEY_MEMBERS).filter((m) => m.id !== id));
    renderMembers();
};

window.deleteScheme = function (id: number) {
    if (prompt("Admin password:") !== ADMIN_PASSWORD) return alert("Incorrect");
    save(KEY_SCHEMES, load<Scheme>(KEY_SCHEMES).filter((s) => s.id !== id));
    renderSchemes();
};

window.deleteImage = function (id: number) {
    if (prompt("Admin password:") !== ADMIN_PASSWORD) return alert("Incorrect");
    save(KEY_IMAGES, load<GalleryImage>(KEY_IMAGES).filter((i) => i.id !== id));
    renderGallery();
};
...
function initForms(): void {
    ...
    if (infoForm) {
        infoForm.addEventListener("submit", (e) => {
            ...
            if (pwd !== ADMIN_PASSWORD) return alert("Incorrect");
